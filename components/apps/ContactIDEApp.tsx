  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setErrors({ message: data.error || 'Server error. Please try again.' })
      }
    } catch {
      ...
      <AnimatePresence mode="wait">
        ...
        {stage === 'code' && (
          ...
          >
            ...
            {/* Always-visible hint */}
            <div className="mx-4 mb-3 px-3 py-2.5 rounded-lg bg-sky-500/10 border border-sky-500/25 flex items-center gap-2.5">
            ...
            {/* Run button */}
            <div className="px-4 md:px-6 pb-4 md:pb-6 border-t border-slate-800 pt-3 md:pt-4 flex flex-col md:flex-row items-center gap-2 md:gap-3">
              <button
                onClick={() => setStage('form')}
                className="flex items-center gap-2 md:gap-3 px-4 md:px-6 py-2.5 md:py-3 rounded-lg bg-sky-500 hover:bg-sky-400 transition-colors text-white text-sm md:text-base font-semibold shadow-lg shadow-sky-500/25 w-full md:w-auto justify-center"
              >
                <span>▶</span>
                <span>Run Code</span>
              </button>
              <span className="text-slate-600 text-xs md:text-sm text-center md:text-left">Execute sendMessage() with your data</span>
            </div>
          </motion.div>
        )}
